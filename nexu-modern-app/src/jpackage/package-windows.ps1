...
if ([string]::IsNullOrWhiteSpace($JarPath)) {
    $JarPath = Join-Path $ProjectRoot "nexu-modern-app\target\nexu-modern.jar"
}
if ([string]::IsNullOrWhiteSpace($Destination)) {
    $Destination = Join-Path $ProjectRoot "nexu-modern-app\target\jpackage"
}
...
Copy-Item -LiteralPath $JarPath -Destination (Join-Path $InputDirectory "nexu-modern.jar")
...
Copy-Item -LiteralPath (Join-Path $ProjectRoot "nexu-modern-app\src\main\resources\nexu-config.properties") `
